...
3. 변경된 UI에서 전환 이벤트 발생 시 전환 지표를 집계한다.
...
```
  pushState()와 replaceState() 설명 이미지 추가
  URL 변경 설명도 추가하기
```
...
```
동적 생성되는 요소 이벤트 핸들러 에러 메세지 이미지 할당
```

_생성되지 않은 요소에 이벤트 핸들러를 할당할 경우_
...
```
  축소 전/후 크키 결과 이미지 추가 + 페이지 로딩 속도 이미지 (표)
```
...
`Etag` 비교 방식에서 브라우저는 응답 헤더에 `Etag`가 포함되어 있을 경우 HTTP 캐싱 시 리소스와 `Etag`를 저장합니다.

이 후 동일한 후속 요청에 캐싱된 리소스가 유효하지 않을 경우 다음과 같은 유효성 검사 과정이 진행됩니다.

<div align="center">

```
  Etag 비교 방식 유효성 검사 다이어그램 이미지 추가
  + 기본적으로 max-age는 설정되어 있지 않다는 점 표시
```
...
<!-- 2. `If-None-Match` 헤더를 포함하는 조건부 요청을 전송합니다.
3. 서버는 조건부 요청에 포함된 `If-None-Match`의 값과 리소스의 `Etag`을 비교하여 -->
